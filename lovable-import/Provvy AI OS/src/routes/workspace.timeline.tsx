            ...
            <div key={g.label}>
              ...
              <div className="relative mt-3 space-y-1 pl-3">
                ...
                {g.events.map((e, i) => {
                  const Icon = e.icon;
                  return (
                    <div key={i} className="relative flex items-start gap-3 rounded-lg py-2.5 pl-4 pr-2 transition-colors hover:bg-secondary/60">
                      <div
                        className={`relative z-10 mt-0.5 grid h-7 w-7 place-items-center rounded-lg ${
                          e.tone === "primary" ? "bg-accent text-accent-foreground" : "bg-secondary text-ink-soft"
                        }`}
                      >

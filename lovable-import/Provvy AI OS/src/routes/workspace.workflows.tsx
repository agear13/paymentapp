...
export const Route = createFileRoute("/workspace/workflows")({
...
const DEPLOYED = [
  {
    ...
    hours: "38 hrs / mo",
    to: "/workspace/workflow/reconciliation" as const,
    primary: true,
  },
];
...
const LIBRARY = [
  ...
  {
    name: "Revenue Sharing Automation",
    ...
    <div className="animate-fade-up space-y-8 pb-16">
      ...
      <section>
        ...
        <div className="mt-3 grid gap-3">
          {DEPLOYED.map((w) => (
            <div
              key={w.name}
              className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-primary/20 bg-card p-5 shadow-card"
            >
              <div className="flex-1">
                ...
                <div className="mt-3 text-xl font-semibold tracking-tight">{w.name}</div>
                <p className="mt-1 max-w-xl text-[13.5px] text-ink-soft">{w.summary}</p>
      ...
      <section>
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {LIBRARY.map((w) => (
            <div key={w.name} className="rounded-2xl border border-border bg-card p-5 shadow-card">
              <div className="text-[15px] font-semibold">{w.name}</div>
              <p className="mt-1 text-[13px] text-ink-soft">{w.summary}</p>

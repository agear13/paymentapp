...
export const Route = createFileRoute("/assessment/start")({
  head: () => ({
    meta: [
      { title: "Where would you like to start? — Provvy" },
      {
        name: "description",
        content:
          "Your Commercial Operating System is configured. Choose the workflow you'd like to begin with today.",
      },
      { property: "og:title", content: "Where would you like to start? — Provvy" },
      {
        property: "og:description",
        content: "Provvy AI recommends the first commercial workflow to open in your workspace.",
      },
...
/** Assessment outcome → recommended first workflow */
...
function StartScreen() {
  ...
  const launch = (card: (typeof CARDS)[number]) => {
    ...
    setTimeout(() => navigate({ to: card.to }), 480);
  };
          ...
          <div className="rounded-2xl border border-primary/20 bg-card p-5 shadow-card">
            ...
            <div className="mt-6 rounded-2xl border border-primary/20 bg-accent p-4">
              ...
              <div className="mt-2 text-[13px] text-ink-soft">
                Based on your assessment we detected:
              </div>

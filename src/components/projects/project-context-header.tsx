...
export function ProjectContextHeader({ className }: { className?: string }) {
  ...
  const name = summary?.name ?? deal.dealName ?? PRODUCT_TERMINOLOGY.project;
  ...
  return (
    <header className={cn(opSurface('inset', 'flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2'), className)}>
      <div className="min-w-0">
        <p className={opTypeMeta}>{PRODUCT_TERMINOLOGY.projectCoordination}</p>
        <h1 className={cn(opTypePageTitle, 'mt-0.5 truncate')}>{name}</h1>
        <p className={cn(opTypeBodySnug, 'mt-1')}>
          {count === 0
            ? `Add participants to coordinate earnings, obligations, and settlement ${PRODUCT_TERMINOLOGY.forThisProject}.`
      ...
      >
        {PRODUCT_TERMINOLOGY.projectOverview}
      </Link>
    </header>
  );
}

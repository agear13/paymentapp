...
describe('homepage narrative and control principles', () => {
  ...
  it('opens as a payment comparison around an intact search', () => {
    expect(landing).toContain('The Skyscanner for payments.');
    expect(landing).toContain('Compare payment routes, understand the trade-offs');
    expect(landing).toContain('recommendation built around');
    expect(search).toContain("Tell Provvy what you're paying");
    expect(search).toContain('What are you paying, which routes exist');
  ...
  it('holds the brand line and personalisation ask for after a comparison', () => {
    ...
    expect(labels).toContain("Provvy's pick");
    expect(results).toContain('Connect your business');
    expect(results).toContain('Connect your business to make this recommendation personal.');
  ...
  it('grounds the intelligence layer in a catalog snapshot, not fake live data', () => {
    ...
    expect(watchlist).toContain('Rising in attention');
    expect(authorise).toContain('Authorise payment');

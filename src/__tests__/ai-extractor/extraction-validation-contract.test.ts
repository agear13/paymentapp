...
describe('extraction validation contract (prompt vs Zod)', () => {
  ...
  describe('3. parties[].name.value = null', () => {
    it('drops invalid party instead of rejecting entire extraction', () => {
      const payload = {
        ...baselineExtractionPayload(),
        parties: [
          {
      ...
      expect(parsed.uncertainties.some((u) => u.field === 'parties')).toBe(true);
  ...
  describe('contract mismatch summary (remaining non-nullable string fields)', () => {
    it('drops parties with invalid core name/role without rejecting extraction', () => {
      ...
      for (const { label, payload } of cases) {
        ...
        console.info(`[extraction-contract] ${label}: party dropped, extraction preserved`);
      }
    });
  });
});
